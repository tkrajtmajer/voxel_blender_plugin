...
def project_min_dist(images, width, height, depth, merge_technique, threshold = 1.0, use_depth_mapping=False, intensity_threshold=0.3, concavity_depth=0.5, factor = 1, min_region_size=5, keep_concave_regions=True, hollow_grid=False):
    ...
    if use_depth_mapping:
        for view, image in images.items():
            ...
            curr_depth_map = depth_map.calculate_depth(view, image, other_images, width, height, depth)
            final_depth_map = depth_map.estimate_using_gradients(view, image, curr_depth_map, width, height, depth, intensity_threshold, concavity_depth, factor, min_region_size, keep_concave_regions)
